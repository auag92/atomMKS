...
def AtomCenters(coords, box, len_pixel):
    ...
    for dim in range(3):
        atom_centers[:, dim] = pipe(
            coords[:, dim], lambda x: np.around(x * len_pixel)
        )
    return atom_centers
...
def grid_maker(atom, atomic_radii=None, len_pixel=10, full=False, fft=False):
    ...
    max_r = atomic_radii[
        max(atomic_radii, key=atomic_radii.get)
    ]  # max atom radius
    ...
    if fft:
        ...
        generator = generator_fft(
            box_dim=box_dim, len_pixel=len_pixel, full=full, scaler=scaler
        )
    else:
        spheres = atomic_radii.copy()
        generator = generator_edt(
            box_dim=box_dim, len_pixel=len_pixel, full=full, scaler=scaler
        )
