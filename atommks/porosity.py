...
def accessibleRegion(S, atomH, r_h, overlap=0.01):
    ...
    S_mod = (
        imfilter(x_data=(S < 1e-6), f_data=padder(atomH, np.array(S.shape)))
        / vol
    )
    S_mod = S_mod < overlap
    return S_mod
...
def gen_medialAxis(S):
    return pipe(S, lambda s: s.astype(np.uint8), lambda s: sklz(s))
...
def get_pathLength(path, coords):
    ...
    for i, idx in enumerate(path[1:]):
        coord = coords[idx]
        path_length = path_length + np.sqrt(np.sum((coord - coord0) ** 2))
        coord0 = coord
    return path_length
    ...
    for index, idx0 in enumerate(top):
        ...
        for idx in bot:
            ...
            if done:
                ...
                S_1[dx, dy, dz] += 1
                tort.append(get_pathLength(path, coords) / path_length)
